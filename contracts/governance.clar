...
(define-map delegations
  { delegator: principal, property-id: uint }
  principal
)
...
(define-private (calculate-threshold (total-shares uint) (threshold-bps uint))
...
(define-private (get-delegate (delegator principal) (property-id uint))
  (map-get? delegations { delegator: delegator, property-id: property-id })
)
...
(define-public (cast-vote (proposal-id uint) (vote uint))
  (let
    (
      ...
      (voter-shares (get-share-balance (get share-token-contract proposal) tx-sender))
    ...
    (asserts! (< block-height (get end-block proposal)) ERR-VOTING-NOT-ACTIVE)
    ...
    (try! (contract-call? (get share-token-contract proposal) lock-shares tx-sender voter-shares))
    ...
    (map-set proposals proposal-id
      (merge proposal {
        yes-votes: (if (is-eq vote VOTE-YES) (+ current-yes voter-shares) current-yes),
        no-votes: (if (is-eq vote VOTE-NO) (+ current-no voter-shares) current-no),
        abstain-votes: (if (is-eq vote VOTE-ABSTAIN) (+ current-abstain voter-shares) current-abstain),
...
(define-public (execute-proposal (proposal-id uint))
  (let
    (
      (proposal (unwrap! (map-get? proposals proposal-id) ERR-PROPOSAL-NOT-FOUND))
      (total-shares (get-total-shares (get share-token-contract proposal)))
      (quorum-required (calculate-threshold total-shares (var-get quorum-bps)))
    )
    ;; Check voting period has ended
    (asserts! (>= block-height (get end-block proposal)) ERR-VOTING-PERIOD-NOT-ENDED)
...
(define-public (cancel-proposal (proposal-id uint))
  (let
    ...
    (asserts! (or (is-eq tx-sender (get proposer proposal)) (is-admin)) ERR-NOT-AUTHORIZED)
...
(define-public (delegate-vote (property-id uint) (delegate principal))
  (begin
    (map-set delegations { delegator: tx-sender, property-id: property-id } delegate)
...
(define-public (revoke-delegation (property-id uint))
  (begin
    (map-delete delegations { delegator: tx-sender, property-id: property-id })
